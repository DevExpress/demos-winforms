...
namespace DevExpress.DevAV.Views {
    internal static class ToolbarExtension {
        ...
        static void CreateItem<TViewModel>(DevExpress.Utils.MVVM.MVVMContextFluentAPI<TViewModel> fluentAPI, Expression<Action<TViewModel, EmployeeReportType>> commandSelector, TileGroup group, EmployeeReportType value)
            where TViewModel : class {
...
#pragma warning restore DX0025
        ...
        internal static Image GetImageByUri(string resourceType, string imageName, ISvgPaletteProvider palette) {
            ...
            if(uri.HasSvgImage)
                return uri.GetSvgImage(Size.Empty, palette);
            if(uri.HasDefaultImage)
                return uri.GetDefaultImage();
            return null;
        }
    ...
    internal static class ImageUriDictionary {
        internal static string GetUriByFile(string file) {
            if(List.TryGetValue(file, out string path))
                return path;
    ...
    public class LabelTabController {
        ...
        public object EditValue {
            ...
            set {
                editValue = value;
                EditValueChanged?.Invoke(EditValue, EventArgs.Empty);
