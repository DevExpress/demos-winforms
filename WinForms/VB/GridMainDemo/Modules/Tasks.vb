...
Namespace DevExpress.XtraGrid.Demos
    ...
    Public MustInherit Class BindingCollection
        ...
        Private Event listChangedCore As ListChangedEventHandler
    ...
    Public Class Task
        ...
        Public Property Complete As Boolean
            ...
            Set(ByVal value As Boolean)
                fComplete = value
                If fComplete Then fPercentComplete = 100
                If Not fComplete AndAlso fPercentComplete = 100 Then fPercentComplete = 0
                OnListChanged()
            End Set
        End Property
        ...
        Public Property PercentComplete As Integer
            ...
            Set(ByVal value As Integer)
                ...
                If fPercentComplete < 0 Then fPercentComplete = 0
                If fPercentComplete > 100 Then fPercentComplete = 100
        ...
        Private Shared Function GetCategoryByTask(ByVal collection As TasksWithCategories, ByVal task As Task) As String
            ...
            If Equals(ret, "") Then ret = Properties.Resources.NoneString
            Return ret
        End Function
    ...
    Public Class Tasks
        ...
        Friend Shared Function GetRandomGenerator() As Data.Utils.NonCryptographicRandom
            If MainFormHelper.TakeScreens Then Return New Data.Utils.NonCryptographicRandom(1)
            Return Data.Utils.NonCryptographicRandom.Default
        End Function
        ...
        Public Shared Function GetTasksWithPriority(ByVal collection As TasksWithCategories) As Tasks
            ...
            Dim rnd = GetRandomGenerator()
            Dim names As String() = New String() {"Create the gauges demo design", "Review the new Layout Form", "Check XAF Wizard's new pages", "Create icons for the Layout Control's context menu", "Create Bizcard Design for the blog post", "Create Booth design for TechEd 2015", "Add grayscale icons to the Image Gallery"}
            For i As Integer = 0 To MaxTasks - 1
                ret.List.Add(New Task(collection, i + 1, Properties.Resources.Task & (i + 1).ToString(), TutorialConstants.Today.AddDays(rnd.Next(-3, 5))))
        ...
        Public Shared Function GetTasks(ByVal collection As TasksWithCategories) As Tasks
            ...
            Dim rnd = GetRandomGenerator()
            For i As Integer = 0 To MaxTasks - 1
                ret.List.Add(New Task(collection, i + 1, Properties.Resources.Task & (i + 1).ToString(), TutorialConstants.Today.AddDays(rnd.Next(5))))
                If i = 2 Then
    ...
    Public Class Categories
        ...
        Public Shared Function GetCategories(ByVal collection As TasksWithCategories) As Categories
            ...
            Dim names As String() = New String() {Properties.Resources.Business, Properties.Resources.Competitor, Properties.Resources.Favorites, Properties.Resources.Gifts, Properties.Resources.Goals, Properties.Resources.Holiday, Properties.Resources.Ideas, Properties.Resources.International, Properties.Resources.Personal}
    ...
    Public Class TasksWithCategories
        ...
        Public Shared Function GetTasksWithCategories() As TasksWithCategories
            ...
            Dim rnd = Tasks.GetRandomGenerator()
        ...
        Public Shared Function GetTasksWithPriorities() As TasksWithCategories
            Dim ret As TasksWithCategories = New TasksWithCategories(True)
            Dim rnd = Tasks.GetRandomGenerator()
            For i As Integer = 0 To Tasks.MaxTasks - 1
                Dim cat As Category = ret.fCategories(rnd.Next(ret.fCategories.Count))
                If Not ret.HasCategory(ret.fTasks(i), cat) Then ret.List.Add(New Relation(ret.fTasks(i), cat))
        ...
        Public Function HasCategory(ByVal task As Task, ByVal category As Category) As Boolean
            For i As Integer = 0 To Count - 1
                If Me(i).fCategory Is category AndAlso Me(i).fTask Is task Then Return True
            Next
    ...
    Public Class TasksWithCategoriesDatasource
        ...
        Public Sub New(ByVal grid As GridControl)
            fGrid = grid
            fTasks = TasksWithCategories.GetTasksWithPriorities()
    ...
    Public Class GroupingControllerTasksWithCategories
        ...
        Public Sub New(ByVal grid As GridControl)
            ...
            Dim view As GridView = TryCast(fGrid.MainView, GridView)
            If view IsNot Nothing Then AddHandler view.EndGrouping, New EventHandler(AddressOf Grid_Grouping)
            SetDataSource()
        End Sub

        Private Sub Grid_Grouping(ByVal sender As Object, ByVal e As EventArgs)
            fGrid.BeginInvoke(New Windows.Forms.MethodInvoker(Sub() SetDataSource()))
        ...
        Public ReadOnly Property CategoryColumn As GridColumn
            Get
                ...
                For Each column As GridColumn In view.Columns
                    If Equals(column.FieldName, "CategoryName") Then Return column
                Next
