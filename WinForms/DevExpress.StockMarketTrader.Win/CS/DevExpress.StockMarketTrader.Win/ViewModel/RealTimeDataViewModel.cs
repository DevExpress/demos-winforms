...
namespace DevExpress.StockMarketTrader.ViewModel {
    public class RealTimeDataViewModel : ViewModelBase {
        ...
        protected static void InvokeSync(Action action) {
            if(action != null && Application.OpenForms.Count > 0) {
        ...
        List<LiveTileViewModel> topThreeCompanies = new List<LiveTileViewModel>() {
            new LiveTileViewModel(),
            new LiveTileViewModel(),
            new LiveTileViewModel()
        };
        EventHandler currentPriceIndexChanged;
        EventHandler watchListChanged;
        public event EventHandler CurrentPriceIndexChanged {
            add { currentPriceIndexChanged += value; }
            remove { currentPriceIndexChanged -= value; }
        }
        public event EventHandler WatchListChanged {
            add { watchListChanged += value; }
            remove { watchListChanged -= value; }
        }
        ...
        readonly Timer timerCore;
        public RealTimeDataViewModel(Timer timer) {
            CandlesCount = 72;
            Ticks = 5;
            timerCore = timer;
            const int OfflineTimeInterval = 500;
            timerCore.Interval = OfflineTimeInterval;
            timerCore.Tick += UpdateOnTimer;

            model = new RealTimeDataModel();
            model.UpdateFailed += OnUpdateFailed;
            model.Initialized += OnInitialized;
        ...
        void CorrectCurrentDate() {
            ...
            if(delta > -1)
                ...
                delta = currentDate - CandlesCount * Ticks;
                currentDate = delta < 0 ? 0 : delta;
            }
        }
        void StartTimer() {
            InvokeSync(timerCore.Start);
        }
        void StopTimer() {
            InvokeSync(timerCore.Stop);
        }
        void GetDatesAsync() {
        ...
        void GetVolumeDynamicsDataAsync(int current = 0) {
            ...
            int date = current + Ticks * ((CandlesCount - volumeCount) + 1);
            GetVolumeDynamicsDataAsyncCompleted(model.Service.GetStockDataFromPeriodByCompanyList(date, volumeCount, Ticks, companies.ToArray()));
        }
        void GetVolumeDynamicsDataAsyncCompleted(CompanyStockData[] cd) {
            foreach(CompanyStockData companyStockData in cd) {
                var vm = watchLisBindingData.Where(c => c.CompanyName == companyStockData.CompanyName).FirstOrDefault();
                if(vm != null)
        ...
        void GetWatchListBindingDataAsync() {
            currentDate += CandlesCount * Ticks;
        ...
        void GetStockChartBindingDataAsync() {
            GetStockChartBindingDataAsyncCompleted(model.Service.GetCompanyMultipleDataFromPeriod(currentDate, CandlesCount, Ticks, selectedCompanyName));
        }
        ...
        void UpdateCompaniesVolumeAsync() {
            if(TickCount == Ticks || TickCount == 0)
                GetCompaniesVolumeAsync();
        }
        ...
        void GetCompaniesVolumeAsync() {
            int endIndex = currentDate - Ticks;
        ...
        void UpdateSparklineViewModelCompany() {
            var companyName = SelectedCompany == null ? null : SelectedCompany.CompanyName;
            SparklineTileViewModel.CompanyIndex = companyName;
            double delta = 0;
            double increace = CalcIncrease(companyName, ref delta);
        ...
        void UpdateVolumeDynamicsAsyncCompleted(CompanyStockData[] cd) {
            foreach(CompanyStockData companynStockData in cd) {
                try {
                    ...
                    if(vm == null) continue;
                    IList<TradingDataViewModel> volDyn = vm.VolumeDynamics;
        ...
        void TryResetCurrentDate() {
            if(dates != null && currentDate + Ticks > dates.Count - 1) {
                currentDate = 0;
                TickCount = 0;
            }
        }
        ...
        void UpdateWatchList(StockData[] watchListData) {
            int count = 0;
            int isRise = 0;
            oldWatchListData = new ObservableCollection<CompanyTradingDataViewModel>(watchLisBindingData);
            ...
            if(watchListChanged != null) watchListChanged(this, EventArgs.Empty);
        }
        ...
        void UpdateTransactionBindingData() {
            TryResetCurrentDate();
            if(stockChartBindingData.Count < 1)
                return;
        ...
        void RaiseCurrentPriceIndexChanged() {
            if(currentPriceIndexChanged != null) currentPriceIndexChanged(this, EventArgs.Empty);
        }
        void SetSelectedCompany() {
            StopTimer();
            int delta = currentDate - CandlesCount * Ticks;
            currentDate = delta < 0 ? 0 : delta;
        ...
        CompanyTradingData GetCompanyTradingData(StockData newData, DateTime current) {
            return new CompanyTradingData(newData, string.Empty);
        }
        DateTime UpdateDate(DateTime startDate, bool add) {
            ...
            return (add) ? startDate : returnDate;
        }
