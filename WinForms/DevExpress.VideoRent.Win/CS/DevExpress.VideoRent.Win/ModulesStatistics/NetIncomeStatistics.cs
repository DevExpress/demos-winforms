...
namespace DevExpress.VideoRent.Win.ModulesStatistics {
    public partial class NetIncomeStatistics : ChartExportTutorialControl {
        ...
        void UpdateGauge(double value, string text) {
            double[] values = (chart.Series.Count > 0) ? GetValues(chart.Series[0]): new double[0];
            double min = 2500;
            double max = 4500;
            double avg = 3000;
            if(values.Length > 0) {
        ...
        void ReloadReceipts(DateTime startDate) {
            receipts = new XPCollection<Receipt>(Session, new BinaryOperator("Date", startDate, BinaryOperatorType.GreaterOrEqual));
        }
