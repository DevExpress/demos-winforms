    ...
    public class Customer : Person, IComparable {
        ...
        public bool OverdueTodayItemsExist() {
            foreach(Rent rent in ActiveRents)
                if(rent.ActiveType != (int)ActiveRentType.Active) return true;
            return false;
        }
        public decimal Discount {
            get { return (decimal)ReferenceData.CustomerDiscount.GetValue((int)DiscountLevel); }
        }
        public override string FullName {
            get { return GetFullName(FirstName, MiddleName, LastName); }
        }
        public string FullNameLastSorting {
            get { return GetFullName(LastName, FirstName, MiddleName); }
        }
        ...
        public virtual string FullNameAndCardNumber {
            get { return string.Format("{0}({1:d6})", FullName, CustomerId); }
        }
