...
namespace DevExpress.VideoRent.Helpers {
            ...
            lock(SyncRoot) {
                for(int attempt = 1; ; ++attempt) {
                    ...
                    if(attempt > MaxIdGenerationAttempts / 2) {
                        Thread.Sleep(NonCryptographicRandom.Default.Next(MinConflictDelay, MaxConflictDelay));
                    }
                }
            }
        }
    ...
    public class ResourcesHelper {
        public static string GetXmlDbFileFromResource() {
            using(Stream xmlStream = AssemblyHelper.GetEmbeddedResourceStream(typeof(ResourcesHelper).Assembly, "Data.VideoRent.xml", false)) {
                byte[] data = new byte[xmlStream.Length];
                xmlStream.Read(data, 0, data.Length);
                string path = Security.SafePath.GetTempFileName(".xml");
