...
Namespace DevExpress.SalesDemo.Model
	Public Class OleDataProvider
		...
		Protected Function BuildQueryBySales(ByVal start As Date, ByVal [end] As Date, ByVal period As GroupingPeriod) As String
			...
			End If
			Dim query As String = String.Format("SELECT" & vbCrLf & _
"                                                'TOTAL' AS Name," & vbCrLf & _
"	                                            SUM(Total_Cost) AS TotalCost, " & vbCrLf & _
"	                                            SUM(Units) AS Units" & vbCrLf & _
"                                                {0}" & vbCrLf & _
"                                           FROM " & vbCrLf & _
"                                                Sales" & vbCrLf & _
"                                           WHERE " & vbCrLf & _
"                                                Sale_Date BETWEEN @PeriodStart and @PeriodEnd" & vbCrLf & _
"                                           ", groupByExpression)
			If Not String.IsNullOrEmpty(groupBy) Then
		...
		Protected Function BuildJoinQuery(ByVal start As Date, ByVal [end] As Date, ByVal period As GroupingPeriod, ByVal column As GroupColumn) As String
			...
			Dim query As String = String.Format("SELECT" & vbCrLf & _
"	                                        {1} as Name," & vbCrLf & _
"	                                        SUM(Sales.Total_Cost) as TotalCost, " & vbCrLf & _
"	                                        SUM(Sales.Units) as Units" & vbCrLf & _
"                                            {2}" & vbCrLf & _
"                                        FROM " & vbCrLf & _
"                                            Sales" & vbCrLf & _
"                                        INNER JOIN " & vbCrLf & _
"                                            {3} as LinkedTable on Sales.{0} = LinkedTable.Id" & vbCrLf & _
"                                        WHERE " & vbCrLf & _
"                                            Sales.Sale_Date BETWEEN @PeriodStart and @PeriodEnd  " & vbCrLf & _
"                                        GROUP BY" & vbCrLf & _
