...
'INSTANT VB NOTE: The field contact was renamed since Visual Basic does not allow fields to have the same name as other class members:
...
'INSTANT VB NOTE: The field currentKey was renamed since Visual Basic does not allow fields to have the same name as other class members:
		...
		Private Sub NavBar_MouseMove(ByVal sender As Object, ByVal e As MouseEventArgs)
			...
			If info.InLink Then
				tooltip.ShowHint(DirectCast(info.Link.Item.Tag, NavBarData).Contact, e.Location)
			Else
		...
		Private Sub CreateNavBarItems(ByVal group As NavBarGroup)
			...
			Dim link As NavBarItemLink = AddNavBarItem(group, My.Resources.OwnerName, My.Resources.Owner1, GetTasksData(Nothing), Nothing)
        ...
        Private Function GetTasksData(ByVal contact As Contact) As Object
            Dim ret As IEnumerable = From task In DataHelper.Tasks
                                     Where task.AssignTo Is contact
                                     Select task
            Return ret.Cast(Of Task)().ToList()
        End Function
        Private Function AddNavBarItem(ByVal group As NavBarGroup, ByVal caption As String, ByVal image As SvgImage, ByVal data As Object, ByVal contact As Contact) As NavBarItemLink
		...
		Private Sub ShowData(ByVal item As NavBarItem)
			...
			gridControl1.DataSource = DirectCast(item.Tag, NavBarData).Data
			ShowInfo(gridView1)
		End Sub
		...
		Private Sub gridView1_CustomDrawCell(ByVal sender As Object, ByVal e As DevExpress.XtraGrid.Views.Base.RowCellCustomDrawEventArgs) Handles gridView1.CustomDrawCell
			If e.RowHandle < 0 Then
				Return
			End If
'INSTANT VB NOTE: The variable currentTask was renamed since Visual Basic does not handle local variables named the same as class members well:
				...
				Case TagResources.TaskNew
					If CurrentKey IsNot Nothing Then
						...
						If EditTask(task) = DialogResult.OK Then
							...
							Try
								DirectCast(tasks(CurrentKey), List(Of Task)).Add(task)
							Finally
