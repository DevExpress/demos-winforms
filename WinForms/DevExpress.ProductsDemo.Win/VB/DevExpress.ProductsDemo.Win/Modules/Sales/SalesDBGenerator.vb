	...
	Public Class SalesGenerator
		...
		Public Function Run(ByVal connectionString As String) As Boolean
			Using connection = New OleDbConnection(connectionString)
				...
				RaiseStart()
				Try
					Generate(connection, startDate, CInt(Math.Truncate(daysCount)))
				Finally
					RaiseComplete()
				End Try
				connection.Close()
		...
		Private Sub GenerateDay(ByVal command As DbCommand, ByVal day As Date)
			...
			Dim salesPerDay As Integer = random.Next(180, CInt(Math.Truncate(200 * (1 + (totalDays * dailySalesGrowth) / 10))))
			...
			For d As Integer = 0 To generateIntervals.Length - 1
				Dim x As Integer = 0
				Do While x < generateIntervals(d)
...
'ORIGINAL LINE: return ++maxId;
		...
		Private Function GenerateTimeIntervals(ByVal salesPerDay As Integer) As Integer()
			...
			Dim n As Integer = 10
			Do
				Dim num As Integer = random.Next(salesPerInterval - CInt(Math.Truncate(salesPerInterval * 0.8)), salesPerInterval + CInt(Math.Truncate(salesPerInterval * 0.3)))
...
#Region "Progress"
		...
		Private Sub RaiseProgress(ByVal progress As Double)
			RaiseEvent GenerationProgress(Me, New SalesDemo.Model.ProgressEventArgs(CInt(Math.Truncate(100.0 * progress))))
		End Sub
