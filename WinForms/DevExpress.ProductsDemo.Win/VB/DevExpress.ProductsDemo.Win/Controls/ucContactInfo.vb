...
'ORIGINAL LINE: teNotes.Text = tePhone.Text = teAddress.Text = teBirthDate.Text = teEmail.Text = lbName.Text = string.Empty;
				lbName.Text = String.Empty
'INSTANT VB WARNING: An assignment within expression was extracted from the following statement:
'ORIGINAL LINE: teNotes.Text = tePhone.Text = teAddress.Text = teBirthDate.Text = teEmail.Text = lbName.Text
				teEmail.Text = lbName.Text
'INSTANT VB WARNING: An assignment within expression was extracted from the following statement:
'ORIGINAL LINE: teNotes.Text = tePhone.Text = teAddress.Text = teBirthDate.Text = teEmail.Text
				teBirthDate.Text = teEmail.Text
'INSTANT VB WARNING: An assignment within expression was extracted from the following statement:
'ORIGINAL LINE: teNotes.Text = tePhone.Text = teAddress.Text = teBirthDate.Text
				teAddress.Text = teBirthDate.Text
'INSTANT VB WARNING: An assignment within expression was extracted from the following statement:
'ORIGINAL LINE: teNotes.Text = tePhone.Text = teAddress.Text
...
'INSTANT VB NOTE: The variable height was renamed since Visual Basic does not handle local variables named the same as class members well:
				Dim height_Renamed As Integer = CInt(Math.Truncate(CDbl(img.Height) * pePhoto.Properties.ZoomPercent / 100.0)) + 4
				If pePhoto.Properties.ZoomPercent = 100 Then
					lciPhoto.Height = Math.Min(lciPhoto.Width * img.Height \ img.Width, height_Renamed)
				Else
					lciPhoto.Height = height_Renamed
				End If
			Finally
				lockResize = False
			End Try
		...
		Public Property ZoomFactor() As Single
			...
			Set(ByVal value As Single)
				...
				Else
					pePhoto.Properties.ZoomPercent = CInt(Math.Truncate(value * 100))
