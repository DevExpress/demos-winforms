namespace DevExpress.DevAV.ViewModels {
    ...
    partial class EmployeeCollectionViewModel : ISupportMap, ISupportCustomFilters, ISupportNewParent {
        public bool CanNew() {
            IDocumentManagerService service = GetDocumentManagerService();
            if(service == null)
                return false;
            foreach(var doc in service.Documents) {
                var newEmployeeViewModel = doc.Content as EmployeeViewModel;
                if(newEmployeeViewModel != null && newEmployeeViewModel.IsNew())
                    return false;
            }
            return true;
        }
