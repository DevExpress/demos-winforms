namespace DevExpress.DevAV.Presenters {
    ...
    public abstract class TasksCollectionPresenter<TEntity, TID, TUnitOfWork> : BasePresenter<CollectionViewModel<TEntity, TID, TUnitOfWork>> where TEntity : class where TUnitOfWork : class, IUnitOfWork {
        Action<int> updateUIAction;
        GanttControl ganttControlCore;
        public TasksCollectionPresenter(GanttControl gridControl, CollectionViewModel<TEntity, TID, TUnitOfWork> viewModel, Action<int> updateUIAction) : base(viewModel) {
            this.ganttControlCore = gridControl;
            this.updateUIAction = updateUIAction;
            SubscribeViewModelEvents();
            if (GanttControl != null)
                GanttControl.FocusedNodeChanged += GanttControl_FocusedNodeChanged;
        }
        int lockFocusedNodeChanged = 0;
        private void GanttControl_FocusedNodeChanged(object sender, XtraTreeList.FocusedNodeChangedEventArgs e) {
            if(e.Node != null) {
                lockFocusedNodeChanged++;
                try {
                    var view = (GanttControl)sender;
                    ViewModel.SelectedEntity = e.Node.Focused ? view.GetDataRecordByNode(e.Node) as TEntity : null;
        ...
        protected GanttControl GanttControl {
            get { return ganttControlCore; }
        }
        public void ReloadEntities(Utils.MVVM.MVVMContext context) {
            if (this.context == null)
                this.context = context;
        ...
        void OnReloadEntities(Utils.MVVM.MVVMContext context) {
            if (entitiesBinding == null) {
                entitiesBinding = context.SetBinding(GanttControl, g => g.DataSource, "Entities");
                loadingTrigger = context.SetTrigger<CollectionViewModel<TEntity, TID, TUnitOfWork>, bool>(x => x.IsLoading, (loading) => {
                    if (loading)
            ...
            GanttControl.FindFilterText = null;
            GanttControl.ActiveFilterString = null;
            UpdateEntitiesCountRelatedUI(ViewModel.Entities.Count);
        }
        protected virtual void SetTopRow() {
            GanttControl.ClearSelection();
            if(GanttControl.NodesIterator.Visible.Any()) {
                var firstNode = GanttControl.NodesIterator.Visible.First();
                GanttControl.FocusedNode = firstNode;
                GanttControl.SelectNode(firstNode);
                var nodeEntity = GanttControl.GetDataRecordByNode(firstNode) as TEntity;
