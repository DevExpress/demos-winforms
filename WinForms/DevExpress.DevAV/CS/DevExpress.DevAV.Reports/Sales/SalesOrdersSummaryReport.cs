...
namespace DevExpress.DevAV.Reports {
	public class SalesOrdersSummaryReport : DevExpress.XtraReports.UI.XtraReport {
		...
		void xrTableCell23_SummaryRowChanged(object sender, EventArgs e) {
			...
			if(!storeSales.TryGetValue(info.StoreId, out storeInfo))
				storeSales.Add(info.StoreId, storeInfo = new StoreInfo(info.StoreCity, info.StoreCustomerName));
			storeInfo.TotalSales += info.Total;
		}
