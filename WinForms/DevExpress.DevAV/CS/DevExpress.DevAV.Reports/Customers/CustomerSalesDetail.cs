...
namespace DevExpress.DevAV.Reports {
	public class CustomerSalesDetail : DevExpress.XtraReports.UI.XtraReport {
		...
		void xrTableCell50_SummaryRowChanged(object sender, EventArgs e) {
			...
			decimal current;
			if(storeSales.TryGetValue(currentStore, out current))
				storeSales[currentStore] = current + total;
			else storeSales.Add(currentStore, total);
		}
		void xrTableCell50_SummaryGetResult(object sender, SummaryGetResultEventArgs e) {
			e.Result = storeSales.Count == 0
				? " - " : storeSales.Aggregate((x, y) => x.Value > y.Value ? x : y).Key.City;
			e.Handled = true;
		}
