...
namespace DevExpress.DevAV.Reports {
	public class CustomerSalesSummaryReport : DevExpress.XtraReports.UI.XtraReport {
		...
		void xrLabel4_BeforePrint(object sender, System.ComponentModel.CancelEventArgs e) {
		...
		void CustomerSalesSummary_DataSourceDemanded(object sender, EventArgs e) {
		...
		void xrTableCell23_SummaryRowChanged(object sender, EventArgs e) {
			CustomerSaleDetailOrderItemInfo currentInfo = (CustomerSaleDetailOrderItemInfo)GetCurrentRow();
			StoreInfo storeInfo = null;
			if(!storeSales.TryGetValue(currentInfo.StoreId, out storeInfo))
				storeSales.Add(currentInfo.StoreId, storeInfo = new StoreInfo(currentInfo.StoreCity));
			storeInfo.TotalSales += currentInfo.Total;
		}
		void xrTableCell23_SummaryGetResult(object sender, SummaryGetResultEventArgs e) {
			if(storeSales.Count > 0)
				e.Result = storeSales.Values.Aggregate((x, y) => x.TotalSales > y.TotalSales ? x : y).City;
			else e.Result = " - ";
			e.Handled = true;
		}
		void xrTableCell23_SummaryReset(object sender, EventArgs e) {
			storeSales.Clear();
		}
		sealed class StoreInfo {
