...
namespace DevExpress.XtraMap.Demos {
    ...
    public partial class MapElements : FooteredMapTutorialControl {
        FlightMapDataGenerator dataGenerator;
        MapInfoPanel mapInfoPanel;
        bool firstLoad = true;

        readonly MapElementsOverlayManager overlayManager;
    ...
    public class FlightMapDataGenerator : IDisposable {
        ...
        static string RandomID(NonCryptographicRandom random) {
            int A = Convert.ToInt32('A');
            int Z = Convert.ToInt32('Z');
            int n0 = Convert.ToInt32('0');
            int n9 = Convert.ToInt32('9');
            return string.Format("{0}{1}{2}{3}{4}{5}",
                Convert.ToChar(random.Next(A, Z)),
                Convert.ToChar(random.Next(A, Z)),
                Convert.ToChar(random.Next(n0, n9)),
                Convert.ToChar(random.Next(n0, n9)),
                Convert.ToChar(random.Next(n0, n9)),
                Convert.ToChar(random.Next(A, Z)));
        }
        static string GetPlaneName(NonCryptographicRandom random, double length) {
        ...
        void LoadPlanes() {
            ...
            while(count > 0) {
                RouteInfo routeInfo = Routes[rnd.Next(0, Routes.Count - 1)];
                ...
                double speed = rnd.Next(600, 850);
                TrajectoryInfo trajectory = new TrajectoryInfo(points, speed);
                string planeName = GetPlaneName(rnd, trajectory.Length);
                PlaneInfo info = new PlaneInfo(planeName, RandomID(rnd), a2.City, a1.City, speed, rnd.Next(7200, 10500), trajectory);
    ...
    public class PlaneInfo : ItemInfoBase {
        static readonly SvgImage icon = SvgImage.FromFile(DemoUtils.GetRelativePath("\\Images\\Plane.svg"));
    ...
    public class MapInfoPanel : IDisposable {
        ...
        public MapInfoPanel(ScaleHelper scaleHelper, MapElementsOverlayManager overlayManager) {
        ...
        void UpdatePlaneInfo() {
            if(selectedPlane == null) {
                overlayManager.SetOverlaysVisibility(false, selectedAirport != null);
                return;
            }
        ...
        void UpdateAirportInfo() {
            if(selectedAirport == null) {
                overlayManager.SetOverlaysVisibility(selectedPlane != null, false);
                return;
            }
