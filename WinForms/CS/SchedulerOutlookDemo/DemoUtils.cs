...
namespace DevExpress.XtraScheduler.Demos {
    public class DemoUtils {
        ...
        static string[] _outlookCalendarPaths = null;
        ...
        public static Stream GetResourceStream(string resourceName) {
            Stream result = System.Reflection.Assembly.GetExecutingAssembly().GetManifestResourceStream("DevExpress.XtraScheduler.Demos." + resourceName);
            if(result != null)
                return result;
            result = System.Reflection.Assembly.GetExecutingAssembly().GetManifestResourceStream(resourceName);
            if(result != null)
                return result;
        ...
        static string GetRelativePath(string name, bool isFolder) {
            string rootPath = Path.GetFullPath(Application.StartupPath);
            string fileName = Path.Combine("Data", name);
            for(int i = 0; i <= 10; i++) {
                try {
                    string relativePath = Path.GetFullPath(Path.Combine(rootPath, fileName));
                    if(IsObjectExists(relativePath, isFolder))
                        return relativePath;
                }
                catch { }
                rootPath += @"\..";
        ...
        public static DataTable GenerateScheduleTasks() {
            ...
            DevExpress.Data.Utils.NonCryptographicRandom rnd = DevExpress.Data.Utils.NonCryptographicRandom.Default;
            for(int i = 0; i < 21; i++) {
