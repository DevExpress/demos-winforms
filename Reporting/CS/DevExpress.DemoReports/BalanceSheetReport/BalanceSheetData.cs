...
namespace XtraReportsDemos.BalanceSheetReport {
    public class BalanceSheetDataItem {
        ...
        public string Type {
            get { return itemType == ItemType.Assets ? "Assets" : "Liabilities and Shareholders Equity"; }
        }
        public string SubType { get; set; }
        public string Name { get; set; }
        public double? Value { get; set; }
        public DateTime Date { get; set; }
        public BalanceSheetDataItem(DateTime date, ItemType itemType, string subType, string name, double? value) {
    ...
    public class BalanceSheetData {
        public static List<BalanceSheetDataItem> GetData() {
            DevExpress.Data.Utils.NonCryptographicRandom random = new DevExpress.Data.Utils.NonCryptographicRandom(2147483);
            List<BalanceSheetDataItem> items = new List<BalanceSheetDataItem>();
            items.AddRange(FillItems(random));
            return items;
        }
        static IEnumerable<BalanceSheetDataItem> FillItems(DevExpress.Data.Utils.NonCryptographicRandom random) {
            List<BalanceSheetDataItem> items = new List<BalanceSheetDataItem>();
        ...
        static double GetRandomValue(DevExpress.Data.Utils.NonCryptographicRandom random, int min, int max) {
